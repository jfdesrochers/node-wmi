...
// Inspired by ppoffice/wmic-js
function parseMethodResults(text) {
  var matched = void 0,
      results = [];
  var mRegExp = /instance of __PARAMETERS\s*\{([\s\S]*)\s}/g;
  var kvRegExp = /\s*(.*?)\s*=\s*(.*?);/g;

  var formatKey = function (k) {
    let keys = k.split(' ')
  ...
  while (matched = mRegExp.exec(text)) {
      ...
      while ((kv = kvRegExp.exec(matched[1])) && kv.length > 2) {
          result[formatKey(kv[1])] = formatValue(kv[2]);
      }
  }
  return result;
}
...
// We use method instead of call, because call is already a reserved word in JavaScript
var setMethod = function(method, cb) {
  if (typeof(method) === 'function') {
    cb = method;
    method = undefined;
  }

  log('Set method property for instance to %s.', method);
  this._params.method = method;

  if (typeof(cb) === 'function') {
    log('setMethod called with callback function. Execute query.');
    this.exec(cb);
  }
...
var exec = function(cb) {
  ...
  var callingMethod = Boolean(this._params.method)
  ...
  cp.on('close', function(code) {
    ...
    if (callingMethod) {
      cb(null, parseMethodResults(stdout))
    } else {
...
var Query = function Query(options, cb) {
  ...
  setMethod.call(this, options.method);
...
Query.prototype.method = setMethod;
